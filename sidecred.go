...
RequestLoop:
	...
	CredentialLoop:
		for _, r := range request.CredentialRequests() {
			...
			for _, resource := range state.GetResourcesByID(p.Type(), r.Name, storeConfig.alias()) {
			...
			state.AddResource(p.Type(), newResource(r, storeConfig.alias(), creds[0].Expiration, metadata))
